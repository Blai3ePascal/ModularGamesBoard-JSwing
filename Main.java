...
public class Main {
	 ...
	 * 
	 * <p>
	 * Numero de obst�culos proporcionados con la opcion -o, o {@code null} si no se
	 * incluye la opcion -o.
	 */
	...
	private static void parseGameOption(CommandLine line) throws ParseException {
		...
		case Ataxx:
			if(obstacles != null){
				...
				} else gameFactory = new AtaxxFactoryExt(obstacles); //Default dimension
			} else gameFactory = new AtaxxFactoryExt(); //Default dimension and obstacles
		break;
		default:
	...
	private static Option constructObstacleOption() {
		return new Option("o", "obstacles", true,
				"Number of obstacles in the board. It must be an int.");
	}
	/**
	...
	private static void parseObsOption(CommandLine line) throws ParseException {
		String obsc = line.getOptionValue("o");
		if (obsc != null){
			try{
				obstacles = Integer.parseInt(obsc);
			}catch(NumberFormatException e){
	...
	public static void startGame() {
		...
		case WINDOW:
			...
			if(!multiviews){
				gameFactory.createSwingView(g, c, null,gameFactory.createRandomPlayer(),null);
			}
			else for (Piece p : pieces) 
				gameFactory.createSwingView(g, c, p,gameFactory.createRandomPlayer(),null);
			break;
		default:
			throw new UnsupportedOperationException("Something went wrong! This program point should be unreachable!");
		}
